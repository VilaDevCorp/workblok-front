...
const getSize = (size?: ModalSize): string => {
    switch (size) {
        ...
        case 'm-longer':
            return 'lg:w-2/5 lg:h-5/6 md:max-h-[800px] sm:max-w-[600px]'
        case 'm-fluid-h':
            return 'w-2/5 max-h-[800px] max-w-[600px]'
        case 'm-squared':
            return 'w-3/4 lg:h-3/4 md:max-h-[450px] max-w-[600px]'
        case 's':
            return 'w-1/2 h-1/2 max-h-600 max-w-1200'
        case 'xs-fluid-h':
            return 'w-2/5 !h-fit max-w-[600px]'
        default:
...
export function VilaModal(props: Props) {
    ...
    return (
        <div className="w-full top-0 left-0 h-full md:h-screen  backdrop-blur-sm flex z-40 max-h-screen justify-center items-center fixed backdrop-contrast-75 " onClick={() => props.onClose()}>
            <div className={`relative flex rounded-lg flex-col h-full md:h-screen px-3 py-3 bg-background-900 ${getSize(props.size)} w-full `} onClick={(e) => e.stopPropagation()}>
                {props.hasHeader &&
